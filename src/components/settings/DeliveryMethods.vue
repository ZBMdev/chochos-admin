...
export default class DeliveryMethods extends Vue {
  ...
  getData() {
    ...
    new DeliveryMethodService("/delivery-methods").getAll()
      .then((data) => {
        this.deliveryMethods = data.map((cat) => new DeliveryMethod(cat))
        this.isLoading = false;
      }).catch((e) => {
        this.toast.add({ severity: "error", summary: "There was an error fetching the delivery methods", detail: "Please check your internet connection and refresh the page" })
        console.log(e);
      });
  }

  editDeliveryMethod(deliveryMethod: DeliveryMethod) {
    this.deliveryMethod = new DeliveryMethod({ ...deliveryMethod });
      ...
      if (this.deliveryMethod.id) {
        ...
        this.deliveryMethodService.update(this.deliveryMethod.id, this.deliveryMethod.toCreateParam())
          .then((newDeliveryMethod) => {
            this.deliveryMethod = new DeliveryMethod(newDeliveryMethod);
